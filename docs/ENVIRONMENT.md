...
| `EMAIL_PROVIDER` | No | `console` | `console` logs instead of sending — fine for local dev, **must** be `resend` (or a future real provider) in production or no emails are actually delivered (production logs `[email:console] NOT SENT`). Password-reset mail is sent straight through this provider; other emails also need Redis + a worker (`docs/DEPLOY_HOSTINGER.md`). |
| `RESEND_API_KEY` | Required if `EMAIL_PROVIDER=resend` | unset | Server-only secret. A Resend API error (unverified domain, bad key) is logged as `email.send_failed`. |
...
| `ECONOMIC_CALENDAR_PROVIDER` | No | `manual` | **Documentary only — nothing reads it.** Which providers actually sync is decided by the enabled `CALENDAR` rows at `/admin/data-sources`. The default feed, `financecalendar` (Finance Calendar, free, no key), registers itself on first sync. `authorized` (Trading Economics) needs `ECONOMIC_CALENDAR_API_KEY` to be a real, licensed key. |
| `ECONOMIC_CALENDAR_AUTO_SYNC` | No | `true` | When the calendar page is viewed and the last sync is older than `ECONOMIC_CALENDAR_SYNC_INTERVAL_MINUTES`, one background sync runs — no Redis, worker or cron needed (`src/services/calendar/auto-sync.ts`). Set `false` to disable (Playwright and CI do). |
| `FINANCE_CALENDAR_API_URL` | No | `https://www.financecalendar.com/wp-json/fc/v1` | Finance Calendar's base URL — override only to point at a mock in tests. |
| `CRON_SECRET` | No | unset | 16+ chars. Enables `GET/POST /api/cron/calendar-sync` (`Authorization: Bearer <secret>`) so an hPanel cron job can keep the calendar fresh with no page views. Unset (or too short) = the route returns 404. `?force=1` skips the "is it due" check. |
...
| `ECONOMIC_CALENDAR_SYNC_INTERVAL_MINUTES` | No | `60` | How stale the calendar may get before the next sync runs (page-view auto-sync and `/api/cron/calendar-sync`); also the basis of the "next sync" estimate at `/admin/data-sources`. |
